...
class FailTest extends AnyFlatSpec with Matchers {
  ...
  "Fail " should "accumulate error message" in {
    ...
    fourthFail.userMessages().size should be(3)
    fourthFail.techMessages().size should be(4)
  }
  ...
  "Fail " should "compose with JsError" in {
    val path_node: PathNode              = KeyPathNode("user")
    val js_path: JsPath                  = JsPath(List(path_node))
    val jsError: SorusDSL.JsErrorContent =
      List((js_path, List(JsonValidationError("Not found in path"))))
    val fail                             = Fail(jsError)
    fail.withEx("Second fail message") should be(Fail(
      "Second fail message",
      Some(\/-(Fail(
